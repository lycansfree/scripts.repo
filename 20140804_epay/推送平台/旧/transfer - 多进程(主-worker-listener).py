...
def probe_alive(host,port):
    try:
        ...
        if return_string == 'Dead':
            status = 1
            translogger.info(host + '工作进程已死')
        elif return_string == 'Zombie':
            status = 1
    ...
    if recv_data.strip('\n\r') == 'status':
        ...
        try:
            ...
            if int(currEtime) - int(lastEtime) > 600 and int(lastEsize) == currEsize:
                os.kill(pid,9)
                raise ValueError('10分钟内本地文件大小未发生变化,判定worker已死')
            send_data = lastEtime
        except OSError:
            send_data = 'Dead'
            errorlogger.critical('worker进程%d已停' % pid)
        except ValueError,e:
            send_data = 'Zombie'
            errorlogger.critical(str(e))
        except Exception,e:
...
def main():
    ...
    global pid_listener,pid_worker,pid_status
    global stordir
    ...
    pid_listener = os.path.join(piddir,'listener.pid')
    pid_worker = os.path.join(piddir,'worker.pid')
