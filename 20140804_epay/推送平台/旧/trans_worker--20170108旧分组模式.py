...
def get_alive(localIP,username,password,sid):
    ...
    try:
        ...
        cur = db.cursor()
        cur.execute(sql)
        reg_res = cur.fetchall()
        cur.close()
        db.close()

        for host,port in reg_res:
            if host == localIP:
                # 说明已注册
                status = 0
            else:
...
def trans_main(localIP):
    ...
    while True:
        ...
        ss,living_host,living_time,reg_res = get_alive(localIP,username,password,sid)
        if ss == 1:
            ...
            if initFlg == 0:
                ...
                while True:
                    ss,living_host,living_time_n,reg_res = get_alive(localIP,username,password,sid)
                    cc = 0
            ...
            if ss == 1:
                translogger.warn('[%s]传输器未分配任务,等待%d秒' % (currEtime,interval))
                time.sleep(interval)
                continue
            else:
                # 初始化全局列表
...
def main():
    ...
    localIP = '192.168.116.1'
    if _localIP != localIP:
        errorlogger.critical('请校验localIP:%s,为确保正确,当前服务直接退出' % _localIP)
        sys.exit(2)
